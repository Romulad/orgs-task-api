...
class TestChangeRoleOwnerView(BaseTestClass):
    """### Flow
    - user need to be authenticated
    - test user get not found when ressource doesn't exist
    - test user with:
        - access allowed on org (owner, can_be_accessed_by) can not change role owner users, 403
        - user with access allowed on role can't change role owner users, 403
        - simple org member can not change role owners, 403
        - test other user can't change role owners, 404
    - validate owner user ids provide:
        - ids should exist
        - user that is making the request should have a full access over owner users specified
    - test org creator or role creator can change role owner users
    - owners are successfully added to obj can_be_accessed_by list after request
    """
    url_name = "roles-change-owners"

    def setUp(self):
        self.owner_user, self.org_creator, self.org = self.create_new_org()
        _, self.role = self.create_new_role(self.org)
        self.Role = self.role.__class__
    ...
    def test_no_creator_user_cant_update_role_access_list(self):
        ...
        can_access_role = self.create_and_activate_random_user()
        self.role.can_be_accessed_by.add(can_access_role)

        user_to_add = self.create_and_activate_random_user()
        user_to_add.can_be_accessed_by.add(
            self.owner_user, self.simple_user, can_access_role, org_member
        )
    ...
    def test_other_user_cant_update_task_access_list(self):
        ...
        for user in users:
            ...
            self.role.refresh_from_db()
            self.assertEqual(
                len(self.role.can_be_accessed_by.all()), 0
            )
    ...
    def test_req_data_validation_ids(self):
        ...
        for test_data in test_datas:
            response = self.auth_post(self.org_creator, test_data, [self.role.id])
    ...
    def test_creators_can_update_access_list(self):
        ...
        for user in [
            ...
            response = self.auth_post(user, req_data, [self.role.id])
            self.assertEqual(response.status_code, self.status.HTTP_204_NO_CONTENT)
            self.role.refresh_from_db()
            access_allowed = self.role.can_be_accessed_by.all()
            self.assertEqual(len(access_allowed), 1)
            self.assertIn(self.simple_user, access_allowed)
            # reset for next user
            self.role.can_be_accessed_by.clear()
    
    def test_user_with_perm_can_update_owners(self):
        ...
        response = self.auth_post(user_with_perm, req_data, [self.role.id])
        self.assertEqual(response.status_code, self.status.HTTP_204_NO_CONTENT)
        self.role.refresh_from_db()
        access_allowed = self.role.can_be_accessed_by.all()
        self.assertEqual(len(access_allowed), 1)

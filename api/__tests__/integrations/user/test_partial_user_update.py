...
class TestPartialUpdateUserView(BaseTestClass):
    """### Flow
    ...
    - test user can update a specific field
    ...
    def test_only_access_allowed_user_can_update_data(self):
        ...
        for test_data in test_datas:
            ...
            response = self.auth_patch(current_user, req_data, [target_user.id])
            ...
            self.assertEqual(data.get('last_name'), 'new_last_name')
            # data is updated
            user_data = User.objects.get(id=target_user.id)
            self.assertEqual(user_data.last_name, 'new_last_name')
    ...
    def test_user_can_update_it_own_data(self):
        ...
        response = self.auth_patch(simple_user, req_data, [simple_user.id])
        self.assertEqual(response.status_code, self.status.HTTP_200_OK)
        # data is updated
